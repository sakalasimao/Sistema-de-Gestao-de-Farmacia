 ...
 *
 */
class Relatorio_estoque_1m extends FPDF
{
  function header(){
    ...
    $this->Cell(190,30, utf8_decode('Relatório do Estoque de Produtos Expirados daqui a um mês'),0,0,'C');
    $this->Ln(20);
    ...
    ON e.fk_produto_estoque = p.id_produto where year(dataExpProduto) = year(now())");
     ...
     while ($res = $cmd->fetch(PDO::FETCH_ASSOC)) {
      ...
      if($barcode == ""){ $barcode = 'SEM CÓDIGO';}else { $barcode = $res['barcode']; }
       ...
       $this->Cell(35,10,utf8_decode($res['expiracao']),1,0,'L');
       $this->Ln();
     }
   }
}

$pdf = new Relatorio_estoque_1m;
$pdf->AliasNbPages();
$pdf->AddPage('L','A4',0);
$pdf->headerTable();
$pdf->viewTable($conn);
